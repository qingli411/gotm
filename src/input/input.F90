...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   public read_obs
! Qing Li, 20171219
   public do_input_spec, register_input_1d_spec
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
   subroutine read_spectrum(unit,nfreq,cols,yy,mm,dd,hh,min,ss,freq, &
                            spec,lines,ios)
!
! !DESCRIPTION:
!  Similar to {\tt read\_profile()} but used for reading wave spectrum
...
!  check if N is consistent with nfreq
   ...
   do i=idx1,idx2,stride
      do
         ...
         if (len_trim(cbuf) > 0 .and. .not. (cbuf(1:1) == '#' .or. cbuf(1:1) == '!')) then
            read(cbuf,*,iostat=ios) freq(i),(spec(i,j),j=1,cols)
            if (ios < 0) ios = 1   ! End-of-file (ios<0) means premature end of line, which is a read error (ios>0) to us
            if (ios /= 0) return
            exit
         end if
